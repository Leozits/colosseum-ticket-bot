...
def test_default_state_shape():
    assert DEFAULT_STATE == {"day_statuses": {}, "consecutive_failures": 0}
...
def test_save_then_load_roundtrips(tmp_path):
    path = str(tmp_path / "state.json")
    state = {"day_statuses": {"2026-09-12": "soldout"}, "consecutive_failures": 1}
...
def test_save_state_writes_valid_json(tmp_path):
    path = str(tmp_path / "state.json")
    save_state(path, {"day_statuses": {}, "consecutive_failures": 0})
    with open(path) as f:
